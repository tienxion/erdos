...
pdftitle={Quantitative obstructions for nearly shortest Egyptian fractions},
pdfauthor={tienxion; AI assistance disclosed in the text}}
...
\begin{document}
\begin{center}
{\LARGE\bfseries Quantitative obstructions for\\[4pt]
nearly shortest Egyptian fractions\par}
\vspace{8pt}
{\large A partial-result note for Erd\H{o}s problem 295\par}
\vspace{5pt}
tienxion \quad\textbullet\quad 4 September 2026
\end{center}
\begin{abstract}
For distinct integers at least \(N\) whose reciprocals sum to one, write
\(D\) for the excess length over \((e-1)N\).
We give an exponential-denominator obstruction when \(D\) is small,
an explicit lower bound of order \(N/\log N\) under each fixed polynomial
denominator ceiling, and stronger constants for subquadratic ceilings.
These restricted statements do not solve the unrestricted problem.
The methods have classical antecedents; novelty is not asserted.
\end{abstract}
...
\qquad b=\frac{e-1}{2}.
\]
All logarithms are natural. Asymptotic assertions concern sequences with \(N\to\infty\). The original problem asks whether the \emph{unrestricted} minimum possible \(D\) tends to infinity.

\section*{Results}

\textbf{Theorem 1 (exponential tail).} If \(D=o(N/\log N)\), then
\[
...
\textbf{Corollary 1.1 (bounded excess).} Fix \(C\ge0\), and put
\[
...
\]
Thus a fixed additive excess forces a bounded number of denominators above \(N^2\), whose product has logarithm at least \((e-o(1))N\). 

\textbf{Corollary 1.2 (a subexponential ceiling).} If \(F=F(N)\) satisfies
\[
...
\textbf{Theorem 2 (a polynomial ceiling).} Fix \(B>1\). Every sequence of representations with \(M\le N^{B+o(1)}\) satisfies
\[
...
\textbf{Theorem 3 (subquadratic multiplicity).} If \(1<B<2\) and \(M\le N^{B+o(1)}\), put \(s=\lceil1/(B-1)\rceil\). Then
\[
...
\]
Thus \(M=o(N^2)\) yields \(d_2=2.59516741\ldots\), whereas Theorem 2 supplies \(c_1=1.79033944\ldots\) at \(N^{2+o(1)}\). Also \(d_s=\log s+O(1)\) as \(s\to\infty\). These are lower-bound constants, not claimed optimal constants.

\textbf{Classical comparison and provenance.} The question comes from Erd\H{o}s and Graham [1, p. 35]. Vose's classical theorem [4], as restated in [5, p. 151, equation (2)], implies
\[
k(N)\le(e-1)N+O(\sqrt N).
\]
Indeed, take the consecutive block immediately before its reciprocal sum first reaches one. Its positive residual is at most the reciprocal of the next denominator, and has reduced denominator with logarithm \(O(N)\). Vose expands the residual into \(O(\sqrt N)\) distinct unit fractions. Each new denominator is at least the next denominator, so these terms are disjoint from the block. This is an attributed classical corollary.

The prime-divisor and positive-numerator methods below have classical antecedents, notably Croot [2] and Martin [3]. The particular formulations and constants were not found in the primary sources inspected, but this limited search does not establish novelty. The original 1971 solution [6] and Burshtein's relevant 2005 paper [7] were not accessible in full during the source audit.

\section*{1. The harmonic loss identity}
...
\section*{2. A bounded-cofactor prime lemma}

\textbf{Lemma.} Fix positive \(\varepsilon,T\). For sufficiently large \(N\), if a prime \(p>\varepsilon N\) divides some element of \(A\cap[N,TN]\), then \(p\) divides some element of \(A\cap(TN,\infty)\).

\textbf{Proof.} Suppose there is no such latter element. Write the selected multiples of \(p\) as \(mp\), with \(m\) belonging to a nonempty subset \(J\subseteq\{1,\ldots,K\}\), where \(K=\lfloor T/\varepsilon\rfloor+1\) is fixed. Their reciprocal sum is
\[
...
This is an elementary form of a classical prime-divisor obstruction, not a newly introduced mechanism. Croot and Martin use quantitatively stronger versions when the maximum denominator is near \(N\); see [2] and [3].
...
\section*{4. A component bound for the number of supported primes}
...
Take one connected component with \(q\) prime vertices and \(r\ge1\) edges, represented by denominators \(a_1,\ldots,a_r\). Write \(j_i\) for the number of distinct large prime factors of \(a_i\), and let \(b_i\) be the integer obtained from \(a_i\) by removing \emph{all} prime-power factors whose primes exceed \(\varepsilon N\). Then
\[
...
\]
This is the required improvement over counting prime factors in each denominator separately. Removing all large prime \emph{powers} in (12a) ensures that repeated factors cause no problem.
...
\section*{7. Evaluation of the constants}
...
\begin{itemize}
\item \(w(x)=1/e\) at \(x=5e/18\); between this point and \(e/3\), \(w(x)=5/(6x)-2/e\).
\item \(w(x)=1/(2e)\) at \(x=e/3\); between \(e/3\) and 1, \(w(x)=1/(2x)-1/e\).
\item For \(t\ge3\), there is no crossing below 1.
...
(1/2,1),(3/2,2),(1,1).
\]
Integrating these pieces gives the boxed expression. To verify that no interval near zero is omitted, \(w(x)>2/e\) for \(x\le1/3\): at the left limit of \(1/3\), use \(3\sum_{j=4}^8 1/j=743/280\) and \(e>8/3\); for the subsequent left limits use \(g(m)\ge g(4)>2/e\). This last inequality follows from \(\log(5/4)<1/4\) and \(e<3\). Monotonicity between jumps then supplies exactly the listed crossings. 


\section*{8. Scope and remaining gap}

Theorem 1 implies that fixed additive excess requires exponential denominators, and Theorems 2 and 3 give growing lower bounds under polynomial denominator ceilings. They do not prove that the unrestricted excess tends to infinity. In particular, boundedly many exponential denominators remain compatible with these results.

The scope of this note is limited to lower bounds under denominator restrictions. No priority claim is made for the formulations in this note.

\textbf{AI assistance.} OpenAI GPT-6 through Codex contributed substantially to the derivation, writing, informal proof reviews, and literature searches. These are informal mathematical proofs, not proof-assistant formalizations.

\section*{References}\begingroup\small\setlength{\parskip}{4pt}

[1] P. Erd\H{o}s and R. L. Graham, \emph{Old and New Problems and Results in Combinatorial Number Theory}, Monographies de L'Enseignement Mathématique 28 (1980), pp. 34-35. \url{https://mathweb.ucsd.edu/~ronspubs/80_11_number_theory.pdf}

[2] E. S. Croot III, \emph{On unit fractions with denominators in short intervals}, Acta Arithmetica 99 (2001), 99-114. In particular, the prime-divisor discussion on p. 105 and Lemma 4. \url{https://arxiv.org/abs/math/9904181}

[3] G. Martin, \emph{Denser Egyptian fractions}, Acta Arithmetica 95 (2000), 231-260. See Section 5 for related prime-power cancellation methods. \url{https://matwbn.icm.edu.pl/ksiazki/aa/aa95/aa9533.pdf}

[4] M. D. Vose, \emph{Egyptian fractions}, Bulletin of the London Mathematical Society 17 (1985), 21-24. Original full proof not inspected; the uniform theorem was checked through [5]. \url{https://doi.org/10.1112/blms/17.1.21}

[5] G. Tenenbaum and H. Yokota, \emph{Length and denominators of Egyptian fractions, III}, Journal of Number Theory 35 (1990), 150-156. \url{https://tenenb.perso.math.cnrs.fr/PPP/Egyptian.pdf}

[6] H. D. Ruderman, P. Erd\H{o}s and E. G. Straus, \emph{E2232}, American Mathematical Monthly 78 (1971), 302-303. Bibliographic reference; full text not inspected. \url{https://doi.org/10.2307/2317539}

[7] N. Burshtein, \emph{On distinct unit fractions whose sum equals 1}, Discrete Mathematics 300 (2005), 213-217. Bibliographic reference; full text not inspected. \url{https://doi.org/10.1016/j.disc.2004.11.007}
\endgroup
