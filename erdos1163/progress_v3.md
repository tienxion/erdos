...
The independent proof audits in `audit_extraspecial.md` and
`audit_extraspecial_clt.md` check the group, normalizer, exact count,
coefficient constants, conditional Poisson argument, and odd extension.
`verify_extraspecial.py` additionally performs the following finite
checks, recorded in `extraspecial_verification.json`:
